...
thread_set_priority (int new_priority) 
{
  ...
  //Si priority es igual que priorityInit, significa que no existe donacion y se puede cambiar a una nueva prioridad
  //sin ningun problema
  if (curr->priority == curr->priorityInit)
  {
    curr->priority = new_priority;
    curr->priorityInit = new_priority;
  }else{
    //Como ya tiene donacion solo priorityInit se cambia
    curr->priorityInit = new_priority;
  }

  //Tiene que existir otros threads
  if (!list_empty(&ready_list))
  {
    struct thread *siguiente = list_entry(list_begin(&ready_list), struct thread, elem);
    if (siguiente != NULL && curr->priority > new_priority)
    {
      thread_yield();
    }
  }
...
void
priorityDonation (struct thread *donado, int new_priority) 
{
  ...
  */
  ...
  donado->priority = new_priority;

  // Si el siguiente thread tiene una prioridad mayor se hace yield
  if (donado == thread_current() && !list_empty(&ready_list))
  {
